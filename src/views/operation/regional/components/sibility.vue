...
export default {
  ...
  methods: {
    ...
    // 查询加盟商其他责任区域信息
    getOtherArea(noClear = false) {
      console.log(this.form);
      dutyParkFindPositionModelList({
        largeAreaId: this.form.largeAreaId,
        franchiseeId: this.form.franchiseeId,
        dutyParkId: this.form.id || -1
      }).then(res => {
        let amap = this.$refs.MapRegion;
        if (!noClear) {
          amap.clearMap();
        }
        let actArr = [];
